import { Translate } from 'wacom';

export const en = {
	'Go to homepage': 'Go to homepage',
	'Switch to dark mode': 'Switch to dark mode',
	'Switch to light mode': 'Switch to light mode',
	'Open language menu': 'Open language menu',
	'Switch language to': 'Switch language to',
	'Our Services': 'Our Services',
	'Technologies': 'Technologies',
	'How We Work': 'How We Work',
	'Partners': 'Partners',
	'Contact': 'Contact',
	'IT Solutions': 'IT Solutions',
	'Main navigation': 'Main navigation',
	'Delivery / Workflow': 'Delivery / Workflow',
	'Problem / Solution': 'Problem / Solution',
	'Value / Benefits': 'Value / Benefits',
	'Scope / Process': 'Scope / Process',
	'Support & Maintenance': 'Support & Maintenance',
	'Risks & Assumptions': 'Risks & Assumptions',
	'Web Development': 'Web Development',
	'Full-stack websites and web apps built for scale, speed, and maintainability.':
		'Full-stack websites and web apps built for scale, speed, and maintainability.',
	'We move from discovery and UX alignment into interface design, component implementation, integrations, QA, and staged release.':
		'We move from discovery and UX alignment into interface design, component implementation, integrations, QA, and staged release.',
	'Companies need modern web products that load fast, scale cleanly, and support business growth without constant rewrites.':
		'Companies need modern web products that load fast, scale cleanly, and support business growth without constant rewrites.',
	'You get faster launches, cleaner user journeys, stronger conversion paths, and a frontend foundation that remains maintainable over time.':
		'You get faster launches, cleaner user journeys, stronger conversion paths, and a frontend foundation that remains maintainable over time.',
	'Typical scope includes architecture, responsive UI, CMS or API integration, performance tuning, SEO basics, and deployment setup.':
		'Typical scope includes architecture, responsive UI, CMS or API integration, performance tuning, SEO basics, and deployment setup.',
	'After launch we handle fixes, enhancements, dependency updates, and iterative improvements based on user feedback and analytics.':
		'After launch we handle fixes, enhancements, dependency updates, and iterative improvements based on user feedback and analytics.',
	'Good outcomes depend on clear goals, stable requirements, and timely content, design, and stakeholder feedback during delivery.':
		'Good outcomes depend on clear goals, stable requirements, and timely content, design, and stakeholder feedback during delivery.',
	'Back-End Development': 'Back-End Development',
	'Robust server-side systems, APIs, and database layers built for reliability and growth.':
		'Robust server-side systems, APIs, and database layers built for reliability and growth.',
	'We define domain logic, model data flows, implement APIs and background jobs, validate performance, and prepare controlled deployment.':
		'We define domain logic, model data flows, implement APIs and background jobs, validate performance, and prepare controlled deployment.',
	'Products often outgrow fragile backends, slow integrations, and ad hoc data models that block new features and reliability.':
		'Products often outgrow fragile backends, slow integrations, and ad hoc data models that block new features and reliability.',
	'The result is a stable backend layer that supports product growth, cleaner integrations, and more predictable engineering work.':
		'The result is a stable backend layer that supports product growth, cleaner integrations, and more predictable engineering work.',
	'Scope usually covers API design, auth, database architecture, admin logic, third-party integrations, and observability setup.':
		'Scope usually covers API design, auth, database architecture, admin logic, third-party integrations, and observability setup.',
	'We continue with incident fixes, schema changes, endpoint evolution, security updates, and performance tuning after release.':
		'We continue with incident fixes, schema changes, endpoint evolution, security updates, and performance tuning after release.',
	'Success assumes access to domain requirements, legacy constraints, and realistic expectations around migrations and data quality.':
		'Success assumes access to domain requirements, legacy constraints, and realistic expectations around migrations and data quality.',
	'DevOps / Cloud Infrastructure': 'DevOps / Cloud Infrastructure',
	'Cloud environments, deployment pipelines, and infrastructure operations built for stable releases and scalable growth.':
		'Cloud environments, deployment pipelines, and infrastructure operations built for stable releases and scalable growth.',
	'We audit the current setup, define target infrastructure, automate delivery pipelines, configure environments, and validate release safety.':
		'We audit the current setup, define target infrastructure, automate delivery pipelines, configure environments, and validate release safety.',
	'Teams struggle when deployments are manual, environments drift, incidents are hard to trace, and scaling depends on heroics.':
		'Teams struggle when deployments are manual, environments drift, incidents are hard to trace, and scaling depends on heroics.',
	'You gain repeatable releases, better uptime, clearer monitoring, and infrastructure that supports both growth and operational discipline.':
		'You gain repeatable releases, better uptime, clearer monitoring, and infrastructure that supports both growth and operational discipline.',
	'Common work includes cloud setup, CI/CD, containers, secrets handling, reverse proxies, monitoring, and rollback-aware releases.':
		'Common work includes cloud setup, CI/CD, containers, secrets handling, reverse proxies, monitoring, and rollback-aware releases.',
	'Post-launch support covers environment updates, pipeline improvements, monitoring alerts, capacity planning, and incident response.':
		'Post-launch support covers environment updates, pipeline improvements, monitoring alerts, capacity planning, and incident response.',
	'Infrastructure work assumes access to cloud accounts, DNS, hosting policies, and a shared agreement on reliability targets.':
		'Infrastructure work assumes access to cloud accounts, DNS, hosting policies, and a shared agreement on reliability targets.',
	'Smart Contract Development': 'Smart Contract Development',
	'Secure contract logic and on-chain business flows tailored for tokens, apps, and automated transactions.':
		'Secure contract logic and on-chain business flows tailored for tokens, apps, and automated transactions.',
	'We translate business rules into contract architecture, implement and test logic, review edge cases, and support deployment readiness.':
		'We translate business rules into contract architecture, implement and test logic, review edge cases, and support deployment readiness.',
	'Blockchain products fail when contract logic is vague, under-tested, or disconnected from actual token and transaction flows.':
		'Blockchain products fail when contract logic is vague, under-tested, or disconnected from actual token and transaction flows.',
	'You get transparent business rules on-chain, automation of sensitive flows, and contract foundations prepared for audit and launch.':
		'You get transparent business rules on-chain, automation of sensitive flows, and contract foundations prepared for audit and launch.',
	'Scope may include token contracts, staking logic, NFT mechanics, vesting, admin controls, deployment scripts, and integration support.':
		'Scope may include token contracts, staking logic, NFT mechanics, vesting, admin controls, deployment scripts, and integration support.',
	'We assist with test expansion, deployment coordination, contract upgrades where applicable, and issue analysis after launch.':
		'We assist with test expansion, deployment coordination, contract upgrades where applicable, and issue analysis after launch.',
	'This work assumes requirements are specified early because contract changes become expensive once logic is deployed on-chain.':
		'This work assumes requirements are specified early because contract changes become expensive once logic is deployed on-chain.',
	'Blockchain Solutions': 'Blockchain Solutions',
	'Custom blockchain products and chain infrastructure for teams building beyond standard Web3 integrations.':
		'Custom blockchain products and chain infrastructure for teams building beyond standard Web3 integrations.',
	'We align on protocol goals, design chain or dApp architecture, implement infrastructure pieces, test flows, and prepare rollout.':
		'We align on protocol goals, design chain or dApp architecture, implement infrastructure pieces, test flows, and prepare rollout.',
	'Off-the-shelf Web3 tooling is not enough when a product needs custom chain behavior, validator logic, or domain-specific token systems.':
		'Off-the-shelf Web3 tooling is not enough when a product needs custom chain behavior, validator logic, or domain-specific token systems.',
	'You gain a blockchain solution shaped around your product model instead of forcing the business into generic platform limits.':
		'You gain a blockchain solution shaped around your product model instead of forcing the business into generic platform limits.',
	'Typical scope includes Cosmos SDK development, validator setup, RPC services, wallet flows, governance logic, and dApp integration.':
		'Typical scope includes Cosmos SDK development, validator setup, RPC services, wallet flows, governance logic, and dApp integration.',
	'Support can include node operations, upgrades, protocol adjustments, incident handling, and iterative ecosystem improvements.':
		'Support can include node operations, upgrades, protocol adjustments, incident handling, and iterative ecosystem improvements.',
	'Protocol-level work assumes strong requirements, long-term ownership, and realistic planning around security, governance, and maintenance.':
		'Protocol-level work assumes strong requirements, long-term ownership, and realistic planning around security, governance, and maintenance.',
	'Mobile Development': 'Mobile Development',
	'Cross-platform mobile products with polished UX and reliable delivery pipelines.':
		'Cross-platform mobile products with polished UX and reliable delivery pipelines.',
	'We shape mobile requirements, define user flows, build the app in iterations, test on target devices, and manage release readiness.':
		'We shape mobile requirements, define user flows, build the app in iterations, test on target devices, and manage release readiness.',
	'Businesses need mobile products that feel native, stay reliable across devices, and support ongoing product iteration without friction.':
		'Businesses need mobile products that feel native, stay reliable across devices, and support ongoing product iteration without friction.',
	'The outcome is faster mobile delivery, better user retention, and a practical balance between speed, UX quality, and maintainability.':
		'The outcome is faster mobile delivery, better user retention, and a practical balance between speed, UX quality, and maintainability.',
	'Scope often covers UX flows, app architecture, API integration, push notifications, store preparation, and release support.':
		'Scope often covers UX flows, app architecture, API integration, push notifications, store preparation, and release support.',
	'We continue with bug fixes, OS compatibility updates, feature iterations, store submissions, and performance improvements.':
		'We continue with bug fixes, OS compatibility updates, feature iterations, store submissions, and performance improvements.',
	'Strong delivery depends on clear platform priorities, access to device testing, and realistic handling of app store constraints.':
		'Strong delivery depends on clear platform priorities, access to device testing, and realistic handling of app store constraints.',
	'UI/UX Design': 'UI/UX Design',
	'Interfaces that stay clear, conversion-focused, and technically feasible.':
		'Interfaces that stay clear, conversion-focused, and technically feasible.',
	'We begin with product goals and user flows, move through wireframes and prototypes, and finalize interfaces that engineering can implement.':
		'We begin with product goals and user flows, move through wireframes and prototypes, and finalize interfaces that engineering can implement.',
	'Products lose users when interfaces are confusing, inconsistent, or disconnected from actual technical and business constraints.':
		'Products lose users when interfaces are confusing, inconsistent, or disconnected from actual technical and business constraints.',
	'You get clearer user journeys, stronger conversion points, reduced rework, and design decisions grounded in delivery reality.':
		'You get clearer user journeys, stronger conversion points, reduced rework, and design decisions grounded in delivery reality.',
	'Work can include audits, wireframes, UI kits, prototypes, responsive layouts, design systems, and handoff preparation.':
		'Work can include audits, wireframes, UI kits, prototypes, responsive layouts, design systems, and handoff preparation.',
	'We support iteration after launch with UI refinement, additional screens, design QA, and system expansion as the product grows.':
		'We support iteration after launch with UI refinement, additional screens, design QA, and system expansion as the product grows.',
	'Design quality depends on access to product context, fast stakeholder decisions, and alignment on target users and priorities.':
		'Design quality depends on access to product context, fast stakeholder decisions, and alignment on target users and priorities.',
	'Business Analysis / Strategy': 'Business Analysis / Strategy',
	'Scoping, product thinking, and backlog shaping before costly implementation starts.':
		'Scoping, product thinking, and backlog shaping before costly implementation starts.',
	'We gather business context, map processes, define requirements, prioritize scope, and turn ambiguity into an actionable product plan.':
		'We gather business context, map processes, define requirements, prioritize scope, and turn ambiguity into an actionable product plan.',
	'Projects drift when teams begin implementation without clear scope, validated priorities, or agreement on what success looks like.':
		'Projects drift when teams begin implementation without clear scope, validated priorities, or agreement on what success looks like.',
	'This reduces wasted development effort, improves decision quality, and creates a roadmap teams can execute with confidence.':
		'This reduces wasted development effort, improves decision quality, and creates a roadmap teams can execute with confidence.',
	'Scope includes workshops, stakeholder interviews, feature mapping, user stories, backlog shaping, and roadmap planning.':
		'Scope includes workshops, stakeholder interviews, feature mapping, user stories, backlog shaping, and roadmap planning.',
	'We keep supporting delivery through backlog refinement, scope clarification, and decision support as new information appears.':
		'We keep supporting delivery through backlog refinement, scope clarification, and decision support as new information appears.',
	'The analysis is only as good as the business input, stakeholder availability, and willingness to make prioritization tradeoffs.':
		'The analysis is only as good as the business input, stakeholder availability, and willingness to make prioritization tradeoffs.',
	'Quality Assurance': 'Quality Assurance',
	'Manual and structured QA that keeps product quality visible throughout delivery.':
		'Manual and structured QA that keeps product quality visible throughout delivery.',
	'We define test coverage, validate features across environments, track defects, verify fixes, and support release confidence.':
		'We define test coverage, validate features across environments, track defects, verify fixes, and support release confidence.',
	'Without disciplined QA, regressions accumulate, edge cases escape to production, and delivery quality becomes guesswork.':
		'Without disciplined QA, regressions accumulate, edge cases escape to production, and delivery quality becomes guesswork.',
	'You get better release confidence, fewer production surprises, clearer defect visibility, and a more reliable user experience.':
		'You get better release confidence, fewer production surprises, clearer defect visibility, and a more reliable user experience.',
	'Scope can cover test cases, exploratory testing, regression passes, UAT support, browser checks, and defect reporting workflows.':
		'Scope can cover test cases, exploratory testing, regression passes, UAT support, browser checks, and defect reporting workflows.',
	'We continue with regression cycles, release verification, bug reproduction, and quality tracking as the product evolves.':
		'We continue with regression cycles, release verification, bug reproduction, and quality tracking as the product evolves.',
	'Good QA depends on accessible environments, defined acceptance criteria, and sufficient time before release for validation.':
		'Good QA depends on accessible environments, defined acceptance criteria, and sufficient time before release for validation.',
	'System Integration': 'System Integration',
	'Bridging CRMs, APIs, payments, analytics, and back-office tools into one flow.':
		'Bridging CRMs, APIs, payments, analytics, and back-office tools into one flow.',
	'We review source systems, map data exchange, implement integration logic, validate scenarios, and stabilize operational handoffs.':
		'We review source systems, map data exchange, implement integration logic, validate scenarios, and stabilize operational handoffs.',
	'Disconnected systems create duplicate work, inconsistent data, and manual operations that slow down teams and frustrate users.':
		'Disconnected systems create duplicate work, inconsistent data, and manual operations that slow down teams and frustrate users.',
	'Integrated flows reduce operational friction, improve data consistency, and make business processes faster and easier to manage.':
		'Integrated flows reduce operational friction, improve data consistency, and make business processes faster and easier to manage.',
	'Typical scope includes API mapping, middleware logic, webhook handling, auth flows, retries, and integration monitoring.':
		'Typical scope includes API mapping, middleware logic, webhook handling, auth flows, retries, and integration monitoring.',
	'We maintain integrations through API changes, incident fixes, sync tuning, and monitoring improvements over time.':
		'We maintain integrations through API changes, incident fixes, sync tuning, and monitoring improvements over time.',
	'Results depend on third-party API quality, access permissions, documentation accuracy, and stable upstream system behavior.':
		'Results depend on third-party API quality, access permissions, documentation accuracy, and stable upstream system behavior.',
	'Data Solutions': 'Data Solutions',
	'Dashboards, reporting, and storage architecture that turns raw data into decisions.':
		'Dashboards, reporting, and storage architecture that turns raw data into decisions.',
	'We audit data sources, define reporting goals, build pipelines and models, validate outputs, and deliver decision-ready dashboards.':
		'We audit data sources, define reporting goals, build pipelines and models, validate outputs, and deliver decision-ready dashboards.',
	'Teams make weak decisions when data is fragmented, delayed, or impossible to trust across reporting layers.':
		'Teams make weak decisions when data is fragmented, delayed, or impossible to trust across reporting layers.',
	'You gain clearer visibility, faster reporting cycles, and a data foundation that supports planning, operations, and growth.':
		'You gain clearer visibility, faster reporting cycles, and a data foundation that supports planning, operations, and growth.',
	'Scope often includes ETL flows, warehousing, dashboarding, KPI design, reporting layers, and data quality checks.':
		'Scope often includes ETL flows, warehousing, dashboarding, KPI design, reporting layers, and data quality checks.',
	'We continue with pipeline fixes, metric changes, report updates, and source integration adjustments as needs evolve.':
		'We continue with pipeline fixes, metric changes, report updates, and source integration adjustments as needs evolve.',
	'Reliable results depend on source data quality, access to systems, and agreement on how KPIs should be defined.':
		'Reliable results depend on source data quality, access to systems, and agreement on how KPIs should be defined.',
	'Cybersecurity': 'Cybersecurity',
	'Practical security hardening for apps, access flows, and delivery processes.':
		'Practical security hardening for apps, access flows, and delivery processes.',
	'We assess exposure, review architecture and access patterns, prioritize fixes, implement hardening, and support verification.':
		'We assess exposure, review architecture and access patterns, prioritize fixes, implement hardening, and support verification.',
	'Security gaps often come from rushed delivery, weak access control, misconfigured infrastructure, and missing defensive processes.':
		'Security gaps often come from rushed delivery, weak access control, misconfigured infrastructure, and missing defensive processes.',
	'You reduce practical risk, improve trust, and gain a clearer path to safer delivery and operational discipline.':
		'You reduce practical risk, improve trust, and gain a clearer path to safer delivery and operational discipline.',
	'Work can include security review, IAM design, SSO implementation, hardening, policy updates, and remediation guidance.':
		'Work can include security review, IAM design, SSO implementation, hardening, policy updates, and remediation guidance.',
	'We support ongoing security with follow-up reviews, patch guidance, access updates, and incident-related recommendations.':
		'We support ongoing security with follow-up reviews, patch guidance, access updates, and incident-related recommendations.',
	'Security work improves risk posture, but it cannot remove all risk without continuous ownership and process maturity.':
		'Security work improves risk posture, but it cannot remove all risk without continuous ownership and process maturity.',
	'Augmented / Virtual Reality Solutions': 'Augmented / Virtual Reality Solutions',
	'Interactive real-time experiences for training, demos, and product storytelling.':
		'Interactive real-time experiences for training, demos, and product storytelling.',
	'We align on the experience goal, prototype interaction flows, build immersive scenes, validate devices, and refine usability.':
		'We align on the experience goal, prototype interaction flows, build immersive scenes, validate devices, and refine usability.',
	'Standard interfaces are not always enough when a product needs immersive training, spatial visualization, or interactive demonstration.':
		'Standard interfaces are not always enough when a product needs immersive training, spatial visualization, or interactive demonstration.',
	'You get more engaging product experiences, clearer demonstrations, and differentiated digital storytelling for users or clients.':
		'You get more engaging product experiences, clearer demonstrations, and differentiated digital storytelling for users or clients.',
	'Scope may include 3D assets, interaction design, AR or VR flows, device targeting, optimization, and deployment packaging.':
		'Scope may include 3D assets, interaction design, AR or VR flows, device targeting, optimization, and deployment packaging.',
	'We support later updates with content refreshes, compatibility fixes, interaction tuning, and performance improvements.':
		'We support later updates with content refreshes, compatibility fixes, interaction tuning, and performance improvements.',
	'Good outcomes depend on asset quality, device constraints, and realistic expectations around performance and user comfort.':
		'Good outcomes depend on asset quality, device constraints, and realistic expectations around performance and user comfort.',
	'Unity Development': 'Unity Development',
	'Immersive 2D and 3D product builds for apps, prototypes, and interactive platforms.':
		'Immersive 2D and 3D product builds for apps, prototypes, and interactive platforms.',
	'We define gameplay or interaction goals, structure scenes and systems, implement features, test builds, and prepare releases.':
		'We define gameplay or interaction goals, structure scenes and systems, implement features, test builds, and prepare releases.',
	'Interactive products require real-time logic and graphics pipelines that general app stacks cannot deliver efficiently.':
		'Interactive products require real-time logic and graphics pipelines that general app stacks cannot deliver efficiently.',
	'You gain a faster path to interactive prototypes, training tools, games, or visual experiences built on a mature engine stack.':
		'You gain a faster path to interactive prototypes, training tools, games, or visual experiences built on a mature engine stack.',
	'Typical scope covers mechanics, scene architecture, input systems, networking, shaders, optimization, and deployment setup.':
		'Typical scope covers mechanics, scene architecture, input systems, networking, shaders, optimization, and deployment setup.',
	'We continue with feature updates, platform fixes, content changes, performance tuning, and release maintenance.':
		'We continue with feature updates, platform fixes, content changes, performance tuning, and release maintenance.',
	'Delivery depends on defined technical targets, asset readiness, and clarity on platform limitations and performance budgets.':
		'Delivery depends on defined technical targets, asset readiness, and clarity on platform limitations and performance budgets.',
	'Technical Support Services': 'Technical Support Services',
	'Operational support, bug fixing, upgrades, and steady release management.':
		'Operational support, bug fixing, upgrades, and steady release management.',
	'We review the current system, define support priorities, handle incidents and fixes, and organize maintenance around release stability.':
		'We review the current system, define support priorities, handle incidents and fixes, and organize maintenance around release stability.',
	'Teams need dependable technical support when live products must keep running while bugs, regressions, and change requests continue.':
		'Teams need dependable technical support when live products must keep running while bugs, regressions, and change requests continue.',
	'You get steadier operations, faster issue resolution, and a practical support layer that keeps delivery moving.':
		'You get steadier operations, faster issue resolution, and a practical support layer that keeps delivery moving.',
	'Scope commonly includes issue triage, maintenance work, release support, monitoring review, bug fixing, and update coordination.':
		'Scope commonly includes issue triage, maintenance work, release support, monitoring review, bug fixing, and update coordination.',
	'This service is maintenance-focused by design, covering ongoing support cycles, fixes, and operational continuity.':
		'This service is maintenance-focused by design, covering ongoing support cycles, fixes, and operational continuity.',
	'Strong support outcomes require clear ownership boundaries, access to systems, and realistic response expectations.':
		'Strong support outcomes require clear ownership boundaries, access to systems, and realistic response expectations.',
	'Digital Marketing': 'Digital Marketing',
	'Technical growth support for launch pages, campaigns, and measurable visibility.':
		'Technical growth support for launch pages, campaigns, and measurable visibility.',
	'We define campaign goals, prepare targeting and messaging, launch channels, track results, and refine based on performance data.':
		'We define campaign goals, prepare targeting and messaging, launch channels, track results, and refine based on performance data.',
	'Growth stalls when campaigns are disconnected from product positioning, landing pages, and measurable conversion paths.':
		'Growth stalls when campaigns are disconnected from product positioning, landing pages, and measurable conversion paths.',
	'You gain more focused acquisition efforts, better attribution, and marketing activity tied to actual business outcomes.':
		'You gain more focused acquisition efforts, better attribution, and marketing activity tied to actual business outcomes.',
	'Scope may include channel setup, landing support, ad operations, analytics configuration, reporting, and funnel optimization.':
		'Scope may include channel setup, landing support, ad operations, analytics configuration, reporting, and funnel optimization.',
	'We keep improving campaigns with iteration, targeting updates, reporting reviews, and performance-based adjustments.':
		'We keep improving campaigns with iteration, targeting updates, reporting reviews, and performance-based adjustments.',
	'Marketing outcomes depend on offer strength, budget, audience fit, and the quality of landing and sales follow-through.':
		'Marketing outcomes depend on offer strength, budget, audience fit, and the quality of landing and sales follow-through.',
	'Search Engine Optimization': 'Search Engine Optimization',
	'Search visibility improvements driven by technical structure, content planning, and intent.':
		'Search visibility improvements driven by technical structure, content planning, and intent.',
	'We assess current visibility, identify technical and content gaps, implement improvements, and track search performance over time.':
		'We assess current visibility, identify technical and content gaps, implement improvements, and track search performance over time.',
	'Strong products still go unseen when site structure, intent alignment, and search fundamentals are weak or inconsistent.':
		'Strong products still go unseen when site structure, intent alignment, and search fundamentals are weak or inconsistent.',
	'You get stronger discoverability, better-qualified traffic, and search improvements that support long-term acquisition.':
		'You get stronger discoverability, better-qualified traffic, and search improvements that support long-term acquisition.',
	'Common scope includes audits, technical fixes, on-page improvements, keyword planning, schema work, and reporting.':
		'Common scope includes audits, technical fixes, on-page improvements, keyword planning, schema work, and reporting.',
	'SEO support continues through content guidance, technical follow-up, performance tracking, and adjustment as rankings shift.':
		'SEO support continues through content guidance, technical follow-up, performance tracking, and adjustment as rankings shift.',
	'SEO is affected by competition, search engine changes, and content quality, so results improve over time rather than instantly.':
		'SEO is affected by competition, search engine changes, and content quality, so results improve over time rather than instantly.',
	'Social Media Marketing': 'Social Media Marketing',
	'Platform-specific campaigns and content systems that keep brand communication active.':
		'Platform-specific campaigns and content systems that keep brand communication active.',
	'We choose target channels, plan content and campaigns, launch communication rhythms, and optimize based on audience response.':
		'We choose target channels, plan content and campaigns, launch communication rhythms, and optimize based on audience response.',
	'Brands lose attention when social activity is inconsistent, generic, or disconnected from campaign and business priorities.':
		'Brands lose attention when social activity is inconsistent, generic, or disconnected from campaign and business priorities.',
	'You gain stronger channel consistency, better audience engagement, and more structured social communication around the brand.':
		'You gain stronger channel consistency, better audience engagement, and more structured social communication around the brand.',
	'Scope can include platform planning, campaign setup, content calendars, moderation, reporting, and coordination with paid efforts.':
		'Scope can include platform planning, campaign setup, content calendars, moderation, reporting, and coordination with paid efforts.',
	'We support ongoing execution with publishing support, content iteration, campaign adjustments, and community response handling.':
		'We support ongoing execution with publishing support, content iteration, campaign adjustments, and community response handling.',
	'Performance depends on content quality, response speed, brand clarity, and the reality that social growth takes sustained effort.':
		'Performance depends on content quality, response speed, brand clarity, and the reality that social growth takes sustained effort.',
	'Influencer Marketing': 'Influencer Marketing',
	'Partnership-driven promotion strategies designed for authentic reach and measurable lift.':
		'Partnership-driven promotion strategies designed for authentic reach and measurable lift.',
	'We define campaign goals, shortlist relevant creators, shape collaboration terms, launch activations, and measure outcomes.':
		'We define campaign goals, shortlist relevant creators, shape collaboration terms, launch activations, and measure outcomes.',
	'Influencer campaigns often fail when partner selection is weak, messaging is generic, or tracking is treated as an afterthought.':
		'Influencer campaigns often fail when partner selection is weak, messaging is generic, or tracking is treated as an afterthought.',
	'You get more relevant creator partnerships, stronger credibility, and measurable campaign execution rather than vanity activity.':
		'You get more relevant creator partnerships, stronger credibility, and measurable campaign execution rather than vanity activity.',
	'Work may include creator research, outreach, brief creation, coordination, asset review, and campaign reporting.':
		'Work may include creator research, outreach, brief creation, coordination, asset review, and campaign reporting.',
	'We support future waves with creator relationship management, performance analysis, and iteration on campaign structure.':
		'We support future waves with creator relationship management, performance analysis, and iteration on campaign structure.',
	'Results depend on audience fit, creator authenticity, market timing, and the strength of the product or offer being promoted.':
		'Results depend on audience fit, creator authenticity, market timing, and the strength of the product or offer being promoted.',
	'IT Consulting': 'IT Consulting',
	'Clear technical direction for teams that need decisions, not generic advice.':
		'Clear technical direction for teams that need decisions, not generic advice.',
	'We assess the current situation, identify decision points, review options, and provide practical recommendations teams can execute.':
		'We assess the current situation, identify decision points, review options, and provide practical recommendations teams can execute.',
	'Organizations get stuck when technical choices, delivery issues, or scaling concerns are unclear and internal alignment is weak.':
		'Organizations get stuck when technical choices, delivery issues, or scaling concerns are unclear and internal alignment is weak.',
	'You gain sharper decisions, better technical direction, and less wasted time on uncertain architecture or delivery paths.':
		'You gain sharper decisions, better technical direction, and less wasted time on uncertain architecture or delivery paths.',
	'Scope can include audits, architecture review, stack evaluation, scaling guidance, process analysis, and delivery recommendations.':
		'Scope can include audits, architecture review, stack evaluation, scaling guidance, process analysis, and delivery recommendations.',
	'Consulting can continue through follow-up reviews, implementation guidance, and decision support as the roadmap evolves.':
		'Consulting can continue through follow-up reviews, implementation guidance, and decision support as the roadmap evolves.',
	'Advice only creates value when stakeholders act on it and share accurate technical, business, and organizational context.':
		'Advice only creates value when stakeholders act on it and share accurate technical, business, and organizational context.',
	'Chief Technology Officer Services': 'Chief Technology Officer Services',
	'Hands-on leadership for architecture, hiring, delivery structure, and execution.':
		'Hands-on leadership for architecture, hiring, delivery structure, and execution.',
	'We work alongside leadership to define technical direction, strengthen engineering structure, and improve delivery accountability.':
		'We work alongside leadership to define technical direction, strengthen engineering structure, and improve delivery accountability.',
	'Growing companies often need senior technical leadership before they are ready to hire a full-time CTO internally.':
		'Growing companies often need senior technical leadership before they are ready to hire a full-time CTO internally.',
	'You get strategic technical guidance, better hiring and process decisions, and stronger alignment between product and engineering.':
		'You get strategic technical guidance, better hiring and process decisions, and stronger alignment between product and engineering.',
	'Typical scope includes architecture oversight, roadmap shaping, hiring support, process design, team mentoring, and vendor review.':
		'Typical scope includes architecture oversight, roadmap shaping, hiring support, process design, team mentoring, and vendor review.',
	'We stay involved through recurring leadership support, delivery reviews, hiring loops, and roadmap recalibration.':
		'We stay involved through recurring leadership support, delivery reviews, hiring loops, and roadmap recalibration.',
	'This works best when leadership gives the role enough access, decision room, and visibility into business priorities.':
		'This works best when leadership gives the role enough access, decision room, and visibility into business priorities.',
	'Client Onboarding / Implementation': 'Client Onboarding / Implementation',
	'Structured onboarding flows that turn signed clients into operational delivery quickly.':
		'Structured onboarding flows that turn signed clients into operational delivery quickly.',
	'We define onboarding steps, configure systems, align stakeholders, prepare documentation, and move new clients into active delivery.':
		'We define onboarding steps, configure systems, align stakeholders, prepare documentation, and move new clients into active delivery.',
	'Signed clients create friction when onboarding is inconsistent, responsibilities are vague, and implementation takes too long.':
		'Signed clients create friction when onboarding is inconsistent, responsibilities are vague, and implementation takes too long.',
	'You get faster activation, smoother handoff into operations, and a more professional client experience from day one.':
		'You get faster activation, smoother handoff into operations, and a more professional client experience from day one.',
	'Scope may include kickoff planning, setup tasks, access coordination, configuration, enablement sessions, and rollout checklists.':
		'Scope may include kickoff planning, setup tasks, access coordination, configuration, enablement sessions, and rollout checklists.',
	'We support post-onboarding stabilization with follow-up sessions, setup corrections, and refinement of the implementation flow.':
		'We support post-onboarding stabilization with follow-up sessions, setup corrections, and refinement of the implementation flow.',
	'Success depends on client responsiveness, internal ownership, and timely access to required systems and decision-makers.':
		'Success depends on client responsiveness, internal ownership, and timely access to required systems and decision-makers.',
	'Human Resource': 'Human Resource',
	'HR support for growing teams that need stronger hiring processes, candidate screening, and team development.':
		'HR support for growing teams that need stronger hiring processes, candidate screening, and team development.',
	'We align on hiring goals, define role expectations, improve screening steps, and support structured recruitment operations.':
		'We align on hiring goals, define role expectations, improve screening steps, and support structured recruitment operations.',
	'Teams grow slowly when recruitment is inconsistent, candidate evaluation is weak, and hiring processes lack technical alignment.':
		'Teams grow slowly when recruitment is inconsistent, candidate evaluation is weak, and hiring processes lack technical alignment.',
	'You get a stronger hiring pipeline, clearer evaluation, and more efficient collaboration between HR and technical leadership.':
		'You get a stronger hiring pipeline, clearer evaluation, and more efficient collaboration between HR and technical leadership.',
	'Scope can include role shaping, sourcing support, screening frameworks, interview design, candidate evaluation, and process improvement.':
		'Scope can include role shaping, sourcing support, screening frameworks, interview design, candidate evaluation, and process improvement.',
	'We continue with hiring process refinement, recruitment operations support, and iteration on evaluation quality as teams scale.':
		'We continue with hiring process refinement, recruitment operations support, and iteration on evaluation quality as teams scale.',
	'Results depend on a realistic hiring market, clear role definitions, and responsive internal stakeholders during recruitment.':
		'Results depend on a realistic hiring market, clear role definitions, and responsive internal stakeholders during recruitment.',
	'Corporate Training / Development': 'Corporate Training / Development',
	'Team upskilling programs designed to improve delivery maturity and technical depth.':
		'Team upskilling programs designed to improve delivery maturity and technical depth.',
	'We assess team needs, design training tracks, deliver workshops or mentoring, and reinforce adoption through follow-up support.':
		'We assess team needs, design training tracks, deliver workshops or mentoring, and reinforce adoption through follow-up support.',
	'Teams stall when delivery maturity, technical depth, or leadership capability do not keep pace with business demands.':
		'Teams stall when delivery maturity, technical depth, or leadership capability do not keep pace with business demands.',
	'You build stronger internal capability, improve team confidence, and reduce dependency on ad hoc knowledge transfer.':
		'You build stronger internal capability, improve team confidence, and reduce dependency on ad hoc knowledge transfer.',
	'Scope may include technical workshops, leadership training, mentoring, role-based learning plans, and capability assessments.':
		'Scope may include technical workshops, leadership training, mentoring, role-based learning plans, and capability assessments.',
	'We support continued growth with follow-up sessions, updated material, mentoring cycles, and progress reviews.':
		'We support continued growth with follow-up sessions, updated material, mentoring cycles, and progress reviews.',
	'Training is most effective when teams have time to apply it and managers actively reinforce new practices.':
		'Training is most effective when teams have time to apply it and managers actively reinforce new practices.',
	'Back to services': 'Back to services',
	'Service Page': 'Service Page',
	'How This Engagement Works': 'How This Engagement Works',
	'Structured around outcomes, not generic delivery':
		'Structured around outcomes, not generic delivery',
	'Each service page is driven from a consistent structure so teams can evaluate scope, execution model, support expectations, and delivery constraints before starting.':
		'Each service page is driven from a consistent structure so teams can evaluate scope, execution model, support expectations, and delivery constraints before starting.',
	'Section': 'Section',
	'Engagement Snapshot': 'Engagement Snapshot',
	'Focus': 'Focus',
	'Delivery, quality, and durable technical choices':
		'Delivery, quality, and durable technical choices',
	'Output': 'Output',
	'Clear scope, visible execution, and production-ready outcomes':
		'Clear scope, visible execution, and production-ready outcomes',
	'Fit': 'Fit',
	'Founders, product teams, and companies with real delivery needs':
		'Founders, product teams, and companies with real delivery needs',
	'Service Section': 'Service Section',
	'Why Teams Pick This Service': 'Why Teams Pick This Service',
	'Everything on this page is generated from the service data itself':
		'Everything on this page is generated from the service data itself',
	'That gives you a stable content model for future expansion: landing summaries, dedicated service pages, navigation, structured sections, and related-offer linking all stay synchronized.':
		'That gives you a stable content model for future expansion: landing summaries, dedicated service pages, navigation, structured sections, and related-offer linking all stay synchronized.',
	'Start a conversation': 'Start a conversation',
	'See our workflow': 'See our workflow',
	'What This Page Uses': 'What This Page Uses',
	'Title, description, image, and tags from the service record':
		'Title, description, image, and tags from the service record',
	'Six structured content sections for page composition':
		'Six structured content sections for page composition',
	'Stable route id for prerendered SSR pages':
		'Stable route id for prerendered SSR pages',
	'Related Services': 'Related Services',
	'Explore adjacent delivery capabilities': 'Explore adjacent delivery capabilities',
	'View all services': 'View all services',
	'Service Missing': 'Service Missing',
	'Service page not found': 'Service page not found',
	'This route does not match any configured service. Return to the homepage and open one of the available service entries.':
		'This route does not match any configured service. Return to the homepage and open one of the available service entries.',
	'Return to services': 'Return to services',
	'Weaving Digital': 'Weaving Digital',
	'Excellence': 'Excellence',
	"Web freelancer's community based in Ukraine with over 10 years in app and web development. Working to create products with the \"Waw!\" aftertaste.":
		"Web freelancer's community based in Ukraine with over 10 years in app and web development. Working to create products with the \"Waw!\" aftertaste.",
	'IT professionals mentored': 'IT professionals mentored',
	'Startups accelerated': 'Startups accelerated',
	'Years of experience': 'Years of experience',
	'Our services': 'Our services',
	'See how we deliver': 'See how we deliver',
	'Custom Development': 'Custom Development',
	'Tailored digital products delivered with modern frameworks and clean engineering.':
		'Tailored digital products delivered with modern frameworks and clean engineering.',
	'Global Reach': 'Global Reach',
	'Remote-first collaboration for founders, teams, and growing companies worldwide.':
		'Remote-first collaboration for founders, teams, and growing companies worldwide.',
	'Lightning Fast': 'Lightning Fast',
	'Short feedback loops, pragmatic delivery, and production-ready implementation.':
		'Short feedback loops, pragmatic delivery, and production-ready implementation.',
	'Scroll down': 'Scroll down',
	'Services We Provide': 'Services We Provide',
	'Comprehensive digital delivery across product, engineering, design, operations, and growth support.':
		'Comprehensive digital delivery across product, engineering, design, operations, and growth support.',
	'Need something specific? We also shape custom delivery models for unique product or business constraints.':
		'Need something specific? We also shape custom delivery models for unique product or business constraints.',
	'Start your project': 'Start your project',
	'Tech Stack': 'Tech Stack',
	'Cutting-Edge Technologies': 'Cutting-Edge Technologies',
	'We use current frameworks, proven infrastructure, and pragmatic tooling to ship faster without sacrificing maintainability.':
		'We use current frameworks, proven infrastructure, and pragmatic tooling to ship faster without sacrificing maintainability.',
	'Our Technology Philosophy': 'Our Technology Philosophy',
	'Innovation First': 'Innovation First',
	'We choose modern tools when they materially improve delivery, quality, or leverage.':
		'We choose modern tools when they materially improve delivery, quality, or leverage.',
	'Scalable Architecture': 'Scalable Architecture',
	'Every decision is shaped around growth, maintainability, and pragmatic future changes.':
		'Every decision is shaped around growth, maintainability, and pragmatic future changes.',
	'Security by Design': 'Security by Design',
	'Secure defaults are part of the delivery process, not a patch layered on later.':
		'Secure defaults are part of the delivery process, not a patch layered on later.',
	'Our Process': 'Our Process',
	'How We Deliver': 'How We Deliver',
	'A clear sequence from discovery through support, tuned for lean collaboration and production-ready outcomes.':
		'A clear sequence from discovery through support, tuned for lean collaboration and production-ready outcomes.',
	'Phase': 'Phase',
	'Project Communication': 'Project Communication',
	'We align on the business context, constraints, desired outcome, and success signals.':
		'We align on the business context, constraints, desired outcome, and success signals.',
	'Initial discovery': 'Initial discovery',
	'Needs review': 'Needs review',
	'Risk mapping': 'Risk mapping',
	'Planning & Estimate': 'Planning & Estimate',
	'Scope is turned into a practical roadmap with delivery stages and technical options.':
		'Scope is turned into a practical roadmap with delivery stages and technical options.',
	'Feature breakdown': 'Feature breakdown',
	'Cost estimate': 'Cost estimate',
	'Execution plan': 'Execution plan',
	'Development': 'Development',
	'Implementation runs in short iterations with visible progress and constant review.':
		'Implementation runs in short iterations with visible progress and constant review.',
	'Agile cycles': 'Agile cycles',
	'Peer review': 'Peer review',
	'Regular demos': 'Regular demos',
	'Testing & QA': 'Testing & QA',
	'We verify product behavior, edge cases, and readiness before shipping to users.':
		'We verify product behavior, edge cases, and readiness before shipping to users.',
	'Manual QA': 'Manual QA',
	'Regression pass': 'Regression pass',
	'Acceptance checks': 'Acceptance checks',
	'Deployment': 'Deployment',
	'Release is handled with environment checks, monitoring, and rollback awareness.':
		'Release is handled with environment checks, monitoring, and rollback awareness.',
	'Release prep': 'Release prep',
	'Infrastructure check': 'Infrastructure check',
	'Launch support': 'Launch support',
	'Support & Growth': 'Support & Growth',
	'After launch we keep refining performance, adding features, and maintaining stability.':
		'After launch we keep refining performance, adding features, and maintaining stability.',
	'Post-launch support': 'Post-launch support',
	'Optimisation': 'Optimisation',
	'Next iterations': 'Next iterations',
	'Trusted With': 'Trusted With',
	'Our partners': 'Our partners',
	'Get In Touch': 'Get In Touch',
	"Let's Start": "Let's Start",
	'Building': 'Building',
	'Ready to transform your ideas into reality? Our team is here to help you navigate your digital journey.':
		'Ready to transform your ideas into reality? Our team is here to help you navigate your digital journey.',
	'Meet Our Team': 'Meet Our Team',
	'Expert developers ready to bring your vision to life':
		'Expert developers ready to bring your vision to life',
	'Multiple Ways to Connect': 'Multiple Ways to Connect',
	'Email Us': 'Email Us',
	'Send us a message anytime': 'Send us a message anytime',
	'Call Us': 'Call Us',
	'Mon-Fri, 9-18': 'Mon-Fri, 9-18',
	'Visit Us': 'Visit Us',
	'Ohiyenka Street, 63, Kamianets-Podilskyi, Khmelnytskyi Region, 32301':
		'Ohiyenka Street, 63, Kamianets-Podilskyi, Khmelnytskyi Region, 32301',
	'Our headquarters': 'Our headquarters',
	'Business Hours': 'Business Hours',
	'Weekend support available': 'Weekend support available',
	'Send us a Message': 'Send us a Message',
	"We'll respond within 24 hours": "We'll respond within 24 hours",
	'Full Name *': 'Full Name *',
	'John Doe': 'John Doe',
	'Email Address *': 'Email Address *',
	'john@example.com': 'john@example.com',
	'Project Details *': 'Project Details *',
	'Tell us about your project requirements, timeline, and goals...':
		'Tell us about your project requirements, timeline, and goals...',
	'Send Message': 'Send Message',
	'Response Time': 'Response Time',
	'Confidential': 'Confidential',
	'Consultation': 'Consultation',
	'Weaving digital solutions that connect, capture, and deliver results. Your trusted partner for comprehensive IT services.':
		'Weaving digital solutions that connect, capture, and deliver results. Your trusted partner for comprehensive IT services.',
	'Services': 'Services',
	'Company': 'Company',
	'Back to home': 'Back to home',
	'Partner Profile': 'Partner Profile',
	'Trusted Partner': 'Trusted Partner',
	'Explore the brand story, the digital work we delivered, and the live channels available for direct contact.':
		'Explore the brand story, the digital work we delivered, and the live channels available for direct contact.',
	'Website': 'Website',
	'Instagram': 'Instagram',
	'Location': 'Location',
	'Open in Google Maps': 'Open in Google Maps',
	'Android': 'Android',
	'Open app': 'Open app',
	'iOS': 'iOS',
	'Open App Store': 'Open App Store',
	'Our Role': 'Our Role',
	'Partner location preview': 'Partner location preview',
	'Open location': 'Open location',
	'Partner Missing': 'Partner Missing',
	'Partner profile not found': 'Partner profile not found',
	'This route does not match any configured partner. Return to the homepage and open one of the available profiles.':
		'This route does not match any configured partner. Return to the homepage and open one of the available profiles.',
	'Return to homepage': 'Return to homepage',
	'Демчук Денис – майстерня кухонь та меблів преміум-класу на замовлення. Ми спеціалізуємось на створенні індивідуальних меблевих рішень для вашого дому: кухні, шафи, гардеробні, комоди, меблі для вітальні та спальні. Виконуємо точний замір, розробляємо унікальний дизайн-проєкт, виготовляємо з якісних матеріалів і забезпечуємо професійний монтаж. Працюємо в Кам’янці-Подільському та по всій Україні. Замовляючи у нас, ви отримуєте не просто меблі — ви отримуєте стиль, комфорт і сервіс на найвищому рівні.':
		'Demchuk Denys is a premium custom kitchen and furniture workshop. We specialize in creating tailored furniture solutions for your home: kitchens, wardrobes, walk-in closets, dressers, and furniture for living rooms and bedrooms. We provide precise measurements, develop a unique design project, manufacture with quality materials, and ensure professional installation. We work in Kamianets-Podilskyi and across Ukraine. By ordering from us, you get more than furniture — you get style, comfort, and top-level service.',
	'Web Art Work — технологічний партнер майстерні Демчука Дениса. Ми відповідаємо за створення сучасного сайту, налаштування CRM‑системи для обробки замовлень та автоматизації процесів, а також допомагаємо з маркетингом: просуванням у соцмережах, SEO‑оптимізацією та брендингом. Наша мета — цифрова трансформація бізнесу клієнта для зростання продажів і покращення сервісу.':
		'Web Art Work is the technology partner of Demchuk Denys workshop. We are responsible for creating a modern website, setting up a CRM system for order processing and workflow automation, and supporting marketing through social media promotion, SEO optimization, and branding. Our goal is the client business digital transformation for sales growth and better service.',
	'Silver Virgi — натяжні стелі на замовлення для сучасних інтер’єрів. Ми виготовляємо та встановлюємо натяжні стелі будь-якої складності: класичні, багаторівневі, з підсвіткою. Пропонуємо індивідуальні рішення для квартир, приватних будинків, офісів і комерційних приміщень. Гарантуємо точний замір, підбір матеріалів, візуалізацію, професійний монтаж і довготривалий результат. ilver Virgi — це про естетику, якість і надійність, які залишаються з вами надовго.':
		'Silver Virgi creates custom stretch ceilings for modern interiors. We manufacture and install stretch ceilings of any complexity: classic, multi-level, and illuminated. We offer tailored solutions for apartments, private houses, offices, and commercial spaces. We guarantee precise measurements, material selection, visualization, professional installation, and long-lasting results. Silver Virgi stands for aesthetics, quality, and reliability that stay with you for years.',
	'Web Art Work — технологічний партнер Silver Virgi. Ми розробили ефективний вебсайт для компанії з виробництва та встановлення натяжних стель, орієнтований на різні сегменти ринку — від приватних клієнтів до дизайнерів інтер’єру та забудовників. Сайт презентує широкий асортимент рішень, приклади реалізованих проєктів і переваги співпраці, водночас забезпечуючи зручність замовлення та комунікації. Інтегрували CRM-систему, автоматизували заявки й супроводжуємо бізнес у маркетингу: SEO-просування, реклама, стратегія присутності в соцмережах. Наша мета — допомогти Silver Virgi масштабуватися та стати лідером у сфері сучасних стельових рішень.':
		'Web Art Work is the technology partner of Silver Virgi. We built an effective website for the company that manufactures and installs stretch ceilings, aimed at different market segments from private clients to interior designers and developers. The site presents a wide range of solutions, examples of completed projects, and collaboration benefits while keeping ordering and communication convenient. We integrated a CRM system, automated leads, and support the business in marketing through SEO, advertising, and social media strategy. Our goal is to help Silver Virgi scale and become a leader in modern ceiling solutions.',
	'Панянка – простір весільної моди, тканин і фурнітури, створений для тих, хто цінує стиль, естетику та індивідуальність. Ми пропонуємо вишукані весільні сукні, які підкреслюють особливі моменти життя, а також великий вибір мережива, тканин і декоративної фурнітури для створення унікального одягу, аксесуарів та авторських виробів. Працюємо з нареченими, дизайнерами та майстрами пошиття по всій Україні. У нас ви знайдете не просто матеріали чи сукню — ви знайдете натхнення, професіоналізм і бездоганний сервіс.':
		'Panianka is a space for bridal fashion, fabrics, and accessories created for people who value style, aesthetics, and individuality. We offer refined wedding dresses that highlight life special moments, along with a large selection of lace, fabrics, and decorative accessories for creating unique garments, accessories, and original pieces. We work with brides, designers, and sewing professionals across Ukraine. Here you will find more than materials or a dress — you will find inspiration, professionalism, and impeccable service.',
	'Web Art Work — технологічний партнер бутика весільної моди. Ми реалізували повноцінний сайт із каталогом продукції, інтегрували CRM‑систему для зручної роботи з клієнтами та автоматизували процеси обліку й обробки замовлень. Окрім цього, супроводжуємо бізнес у маркетинговій частині: налаштування реклами, SMM‑стратегія, SEO‑оптимізація та візуальний брендинг. Наша мета — цифрове посилення бренду, що поєднує естетику, функціональність і прибутковість.':
		'Web Art Work is the technology partner of the bridal fashion boutique. We delivered a full website with a product catalog, integrated a CRM system for smooth client work, and automated order accounting and processing. In addition, we support the business marketing side through advertising setup, SMM strategy, SEO optimization, and visual branding. Our goal is to strengthen the brand digitally while combining aesthetics, functionality, and profitability.',
	'Tpotier — студія весільного та кутюрного текстилю для дизайнерів, ательє та наречених, які цінують вишуканість, індивідуальність і високу майстерність. Ми пропонуємо ретельно дібрані тканини, делікатне мереживо та елегантну декоративну фурнітуру для створення весільних суконь, аксесуарів і унікального авторського одягу. Наша колекція поєднує класичну європейську естетику з сучасними тенденціями, відповідаючи запитам міжнародного ринку. Працюємо з дизайнерами, майстернями та нареченими з усього світу, забезпечуючи натхнення, якість і професійний супровід на кожному етапі.':
		'Tpotier is a studio of bridal and couture textiles for designers, ateliers, and brides who value sophistication, individuality, and high craftsmanship. We offer carefully selected fabrics, delicate lace, and elegant decorative accessories for creating wedding dresses, accessories, and unique designer clothing. Our collection combines classic European aesthetics with modern trends to meet international market expectations. We work with designers, workshops, and brides from around the world, providing inspiration, quality, and professional support at every stage.',
	'Web Art Work — технологічний партнер Tpotier. Ми створили сучасний вебсайт, орієнтований на міжнародний ринок, з багатомовним каталогом продукції, адаптованим під потреби дизайнерів, ательє та наречених з усього світу. Інтегрували CRM‑систему для зручної обробки замовлень і комунікації з клієнтами, автоматизували бізнес-процеси для ефективного управління. Також супроводжуємо проєкт у маркетинговому напрямі: розробка візуального стилю, SEO‑оптимізація, таргетована реклама та стратегія присутності у соцмережах. Наша мета — масштабування бренду Tpotier через цифрові інструменти, які працюють на якість, імідж і продажі.':
		'Web Art Work is the technology partner of Tpotier. We created a modern website aimed at the international market, with a multilingual product catalog adapted to the needs of designers, ateliers, and brides from around the world. We integrated a CRM system for convenient order handling and client communication, and automated business processes for efficient management. We also support the project in marketing through visual style development, SEO optimization, targeted advertising, and social media presence strategy. Our goal is to scale the Tpotier brand through digital tools that support quality, image, and sales.',
	'VR Arena — простір віртуальних розваг для дітей, підлітків і дорослих. У нас ви можете зануритися у світ віртуальної реальності за допомогою сучасних VR‑окулярів і контролерів. Шутери, пригоди, перегони, квести, хорори — для одного чи кількох гравців. Приймаємо індивідуальних відвідувачів, компанії друзів, родини. Пояснюємо, допомагаємо, супроводжуємо — підходить навіть новачкам. VR Arena — це про нові емоції, активний відпочинок і технологічний досвід, який запам’ятається.':
		'VR Arena is a space for virtual entertainment for children, teenagers, and adults. Here you can immerse yourself in virtual reality using modern VR headsets and controllers. Shooters, adventures, racing, quests, and horrors are available for one or several players. We welcome individual visitors, groups of friends, and families. We explain, help, and guide you, so it works even for beginners. VR Arena is about new emotions, active leisure, and a memorable technology experience.',
	'Web Art Work — технологічний партнер VR Arena. Ми створили вебсайт для простору віртуальної реальності, який поєднує розваги, технології та зручність запису на ігрові сесії. Сайт орієнтований на індивідуальних відвідувачів, компанії друзів, родини та організаторів заходів — від днів народження до корпоративів. Презентуємо перелік ігор, умови відвідування, фото з івентів і можливості бронювання. Інтегрували CRM‑систему для обробки замовлень, автоматизували процеси, запустили рекламні кампанії та розробили digital‑стратегію. Наша мета — допомогти VR Arena масштабуватися, розширити аудиторію та стати провідним гравцем на ринку віртуальних розваг.':
		'Web Art Work is the technology partner of VR Arena. We built a website for the virtual reality venue that combines entertainment, technology, and convenient booking for gaming sessions. The site is aimed at individual visitors, groups of friends, families, and event organizers, from birthdays to corporate events. It presents the game catalog, visit conditions, event photos, and booking options. We integrated a CRM system for order handling, automated processes, launched advertising campaigns, and developed a digital strategy. Our goal is to help VR Arena scale, broaden its audience, and become a leading player in the virtual entertainment market.',
	'Jack Hoker — майстерня виробів зі шкіри з понад 20‑річним досвідом. Ми виготовляємо сумки, гаманці, ремені, рюкзаки та аксесуари з натуральної шкіри, поєднуючи традиції ручної роботи з актуальним дизайном. У кожному виробі — якість перевірених матеріалів, увага до деталей і характерний стиль. Створюємо речі, які служать роками та з часом лише набувають шарму. Працюємо для тих, хто цінує функціональність, автентичність і майстерність, перевірену часом.':
		'Jack Hoker is a leather goods workshop with more than 20 years of experience. We create bags, wallets, belts, backpacks, and accessories from genuine leather, combining handcrafted tradition with contemporary design. Every product carries the quality of trusted materials, attention to detail, and a distinctive style. We make items that last for years and gain even more character over time. We work for people who value functionality, authenticity, and craftsmanship proven by time.',
	'Web Art Work — технологічний партнер Jack Hoker. Ми створили сучасний вебсайт для бренду зі шкіряними виробами, що поєднує автентичність ручної роботи з актуальним цифровим досвідом. Інтерфейс адаптовано під потреби клієнтів, які цінують якість, майстерність і лаконічний стиль. Впровадили систему онлайн-замовлень, оптимізовану для зручної комунікації та обробки запитів, а також підключили CRM для ефективного управління замовленнями. Просуваємо бренд через SEO, візуальну стратегію, таргетовану рекламу та контент у соцмережах. Наша мета — розширити аудиторію Jack Hoker і зміцнити імідж бренду, що втілює цінність часу, традицій і якості.':
		'Web Art Work is the technology partner of Jack Hoker. We created a modern website for the leather goods brand that combines handcrafted authenticity with a relevant digital experience. The interface is adapted for clients who value quality, craftsmanship, and concise style. We implemented an online ordering system optimized for convenient communication and request processing, and connected a CRM for effective order management. We promote the brand through SEO, visual strategy, targeted advertising, and social media content. Our goal is to expand the Jack Hoker audience and strengthen the brand image built around the value of time, tradition, and quality.',
	'PORTIER CAKE — студія тортів, десертів і кенді-барів преміум-класу. Ми спеціалізуємось на створенні вишуканих солодощів з натуральних інгредієнтів: весільні торти, десерти на замовлення, святкові кенді-бари. Працюємо з індивідуальними побажаннями, підбираємо стильне оформлення та дбаємо про бездоганний смак кожного виробу. Готуємо з любов’ю та увагою до деталей у Кам’янці-Подільському. Замовляючи у нас, ви отримуєте не просто десерт — ви отримуєте емоцію, естетику і солодку насолоду на найвищому рівні.':
		'PORTIER CAKE is a premium cake, dessert, and candy-bar studio. We specialize in creating refined sweets from natural ingredients: wedding cakes, custom desserts, and праздничні candy bars. We work with individual wishes, choose stylish presentation, and care about the flawless taste of every product. We prepare everything with love and attention to detail in Kamianets-Podilskyi. When you order from us, you get more than a dessert — you get emotion, aesthetics, and sweet pleasure at the highest level.',
	'Web Art Work — технологічний партнер PORTIER CAKE. Ми створили сучасний сайт, щоб клієнтам було зручно переглядати асортимент, оформлювати замовлення та отримувати актуальну інформацію. Також налаштували CRM-систему для обліку замовлень і комунікації з клієнтами, автоматизували частину бізнес-процесів. Окрім технічної підтримки, супроводжуємо проєкт у сфері маркетингу. Наша мета — допомогти бренду розвиватися онлайн так само елегантно, як виглядають і смакують їхні десерти.':
		'Web Art Work is the technology partner of PORTIER CAKE. We created a modern website so clients can conveniently browse the assortment, place orders, and get up-to-date information. We also configured a CRM system for order tracking and client communication and automated part of the business processes. Alongside technical support, we support the project in marketing. Our goal is to help the brand grow online as elegantly as its desserts look and taste.',
	'UA Code — мобільний застосунок для вивчення програмування українською мовою. Ми створили зручну платформу для тих, хто хоче опанувати програмування з нуля, використовуючи україномовний синтаксис та зрозумілі пояснення. Застосунок поєднує теорію, практичні завдання й інтерактивні приклади. До додатку також входить навчальна книга для системного засвоєння матеріалу. UA Code — це перший крок до ІТ-сфери для школярів, студентів і всіх, хто прагне розвиватись у технологічному середовищі, не втрачаючи зв’язку з рідною мовою. Вивчай код рідною — твори майбутнє своє.':
		'UA Code is a mobile application for learning programming in Ukrainian. We created a convenient platform for people who want to learn programming from scratch using Ukrainian-language syntax and clear explanations. The app combines theory, practical tasks, and interactive examples. It also includes a study book for systematic learning. UA Code is a first step into IT for school students, university students, and everyone who wants to grow in the technology space without losing touch with their native language. Learn code in your own language and shape your own future.',
	'Web Art Work — технологічний партнер освітнього проєкту UA Code. Ми розробили мобільний застосунок для вивчення програмування українською мовою, а також створили навчальну структуру з інтерактивними вправами, тестами та теоретичними блоками. Забезпечили повний цикл: від UI/UX дизайну до розробки та публікації в маркетплейсах. Також супроводжуємо проєкт у напрямках маркетингу, SEO та брендингу. Наша мета — зробити цифрову освіту доступною, якісною та по-справжньому українською.':
		'Web Art Work is the technology partner of the educational project UA Code. We developed a mobile application for learning programming in Ukrainian and built the learning structure with interactive exercises, tests, and theory blocks. We delivered the full cycle from UI/UX design to development and marketplace publication. We also support the project in marketing, SEO, and branding. Our goal is to make digital education accessible, high-quality, and genuinely Ukrainian.',
} as const;

export type EnglishTranslationKey = keyof typeof en;

export const enTranslates: Translate[] = Object.entries(en).map(([sourceText, text]) => ({
	sourceText,
	text,
}));
